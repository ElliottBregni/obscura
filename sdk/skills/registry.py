...
class SkillRegistry:
    ...
    def list_capabilities(self, skill_name: str | None = None) -> list[dict[str, Any]]:
        ...
        skills_to_check: list[Skill] = (
            [self._skills[skill_name]] if skill_name else list(self._skills.values())
        )
    ...
    def discover(self, query: str) -> list[dict[str, Any]]:
        ...
        for skill in self._skills.values():
            ...
            skill_match = (
                ...
                or any(query in tag.lower() for tag in skill.metadata.tags)
            )

            for cap in skill.capabilities:
                cap_match = (
                    query in cap.name.lower() or query in cap.description.lower()
                )
    ...
    async def execute(self, capability_path: str, params: dict[str, Any]) -> Any:
