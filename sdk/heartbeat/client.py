"""
sdk.heartbeat.client — Agent heartbeat client.
...
class AgentHeartbeatClient:
    ...
    async def start(self) -> None:
        ...
        if self._running:
            logger.warning(f"HeartbeatClient for {self.config.agent_id} already running")
            return
            ...
            try:
                ...
                if response.status_code < 400:
                    ...
                    if not self._connected:
                        ...
                        logger.info(f"Connected to heartbeat monitor for {self.config.agent_id}")
                    
                    logger.debug(f"Heartbeat sent successfully for {self.config.agent_id}")
                    return True
                else:
                    logger.warning(
    ...
    async def _build_heartbeat(self) -> Heartbeat:
        ...
        message = None
        if not self._connected:
            message = f"Disconnected from monitor ({self._consecutive_failures} failures)"
    ...
    async def send_status_update(self, status: HealthStatus, message: Optional[str] = None) -> bool:
        """
    ...
    async def register(
        ...
        **kwargs: Any,
        ...
        if agent_id in self._clients:
            logger.warning(f"Agent {agent_id} already registered, stopping existing client")
