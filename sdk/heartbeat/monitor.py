...
class HeartbeatMonitor:
        ...
        for agent_id in agents:
            try:
                ...
                if agent_id in self._last_check:
                    if (now - self._last_check[agent_id]).total_seconds() < self._check_interval:
                        continue
    ...
    async def _compute_health(self, agent_id: str, record: HealthRecord) -> HealthStatus:
        """
    ...
    async def _handle_status_change(
        ...
        if record:
            ...
            message = self._generate_alert_message(agent_id, old_status, new_status, record)
    ...
    async def register_agent(self, agent_id: str, expected_interval: int = 30) -> None:
        ...
        self._transitions[agent_id] = HealthStatusTransition(agent_id)
        logger.debug(f"Registered agent {agent_id} for monitoring (interval: {expected_interval}s)")
