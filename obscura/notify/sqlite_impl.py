...
class SQLiteStorage:
    ...
    async def get_message(self, message_id: str) -> Message | None:
        cur = await self._conn.execute(
            "SELECT id,user_id,channel,payload,status,attempts FROM messages WHERE id=?",
            (message_id,),
        )
    ...
    async def list_pending(self, limit: int = 100) -> list[Message]:
        cur = await self._conn.execute(
            "SELECT id,user_id,channel,payload,status,attempts FROM messages WHERE status IN ('queued','retry') ORDER BY created_at ASC LIMIT ?",
            (limit,),
        )
    ...
    async def update_status(
        ...
        if attempts is None:
            ...
            await self._conn.execute(
                "UPDATE messages SET status=?, attempts=? WHERE id=?",
                (status, attempts, message_id),
            )
        await self._conn.commit()
