...
def _persist_transcript(
    ...
    # 2) File system (JSONL — one JSON object per line)
    try:
        ...
        ts = int(time.time())
        path = transcript_dir / f"chat_{backend}_{ts}.jsonl"
        lines: list[str] = []
    ...
    async def _run_chat() -> None:
        ...
        async with ObscuraClient(
            ...
            if mode == "native":
                ...
                sid = session_ref.session_id if session_ref else _uuid.uuid4().hex
...
def passthrough(vendor: str, vendor_args: tuple[str, ...], capture: bool) -> None:
    ...
    if capture:
        ...
        async def _run_captured() -> None:
            ...
            # Persist transcript to file (JSONL)
            if transcript_lines:
                try:
                    import json as _json
                    from pathlib import Path
                    ...
                    for line in transcript_lines:
                        ...
                        jsonl_lines.append(_json.dumps(record, ensure_ascii=False))
                    path.write_text("\n".join(jsonl_lines)[:50000])
                    console.print(f"[dim]Transcript saved to {path}[/]")
                except Exception:
                    pass
