...
@tool(
    ...
    ),
    {
        ...
        "properties": {
            "operation": {
                "type": "string",
                "enum": ["navigate", "get_content", "screenshot", "click", "fill", "evaluate"],
            ...
            "value": {"type": "string", "description": "Value to fill (for 'fill') or JS expression (for 'evaluate')."},
            "path": {"type": "string", "description": "File path to save screenshot (for 'screenshot')."},
        },
        "required": ["operation"],
    },
)
async def web_browser(
    ...
    try:
        ...
        return json.dumps({"ok": False, "error": "browser_unavailable", "detail": str(exc)})
    ...
    try:
        ...
        if operation == "evaluate":
            if not value:
                return json.dumps({"ok": False, "error": "missing_value", "detail": "Provide JS expression in 'value'"})
