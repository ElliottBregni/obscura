...
class TestBroadcastEvent:
    ...
    async def test_broadcast_event_no_clients(self):
        original = ws_mod._broadcast_clients.copy()
        ws_mod._broadcast_clients.clear()
        try:
            await ws_mod.broadcast_event("test_event", {"key": "value"})
        finally:
            ws_mod._broadcast_clients.extend(original)
    ...
    async def test_broadcast_event_sends_to_clients(self):
        ...
        try:
            client1 = AsyncMock()
            client2 = AsyncMock()
            ws_mod._broadcast_clients.extend([client1, client2])
    ...
    async def test_broadcast_event_removes_disconnected(self):
        ...
        try:
            ...
            ws_mod._broadcast_clients.extend([good_client, bad_client])

            await ws_mod.broadcast_event("test", {})

            good_client.send_json.assert_awaited_once()
            assert bad_client not in ws_mod._broadcast_clients
            assert good_client in ws_mod._broadcast_clients
        finally:
            ws_mod._broadcast_clients.clear()
            ws_mod._broadcast_clients.extend(original)
...
class TestNotifyMemoryChange:
    ...
    async def test_no_namespace(self):
        original = ws_mod._memory_watch_clients.copy()
        ws_mod._memory_watch_clients.clear()
        try:
            await ws_mod.notify_memory_change("missing", "set", "key1")
        finally:
            ws_mod._memory_watch_clients.clear()
            ws_mod._memory_watch_clients.update(original)
    ...
    async def test_notify_sends_to_watchers(self):
        ...
        try:
            client1 = AsyncMock()
            client2 = AsyncMock()
            ws_mod._memory_watch_clients["ns1"] = [client1, client2]
    ...
    async def test_notify_removes_disconnected(self):
        ...
        try:
            ...
            ws_mod._memory_watch_clients["ns2"] = [good_client, bad_client]

            await ws_mod.notify_memory_change("ns2", "delete", "k1")

            good_client.send_json.assert_awaited_once()
            assert bad_client not in ws_mod._memory_watch_clients["ns2"]
            assert good_client in ws_mod._memory_watch_clients["ns2"]
        finally:
            ws_mod._memory_watch_clients.clear()
            ws_mod._memory_watch_clients.update(original)
    ...
    async def test_notify_different_namespaces(self):
        ...
        try:
            ...
            ws_mod._memory_watch_clients["ns1"] = [ns1_client]
            ws_mod._memory_watch_clients["ns2"] = [ns2_client]
