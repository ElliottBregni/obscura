...
class TestDomain1InRepoSync:
    ...
    def test_sync_creates_real_dirs(
        ...
        assert (mock_repo / ".github").is_dir() and not (mock_repo / ".github").is_symlink(), \
            ".github should be a real directory at root"
        assert (mock_repo / ".claude").is_dir() and not (mock_repo / ".claude").is_symlink(), \
    ...
    def test_platform_has_scoped_skills(
        ...
        plat_files = sorted(
            ...
            for f in plat_github.rglob("*") if f.is_symlink()
        )
    ...
    def test_partview_has_own_skills(
        ...
        pv_files = sorted(
            ...
            for f in pv_github.rglob("*") if f.is_symlink()
        )
        ...
        assert "agent.md" in pv_files, \
            "partview_core/.github should inherit agent.md"
    ...
    def test_partview_no_platform_skills(
        ...
        for link in pv_github.rglob("*"):
            if link.is_symlink():
                ...
                if rel_to_pv not in (
                    "agent.md", "agent.json", "config.json",
