...
class TestCommandExecution:
    ...
    async def test_cmd_export_json(self) -> None:
        ...
        export_path = Path.home() / ".obscura" / "exports" / f"{ctx.session_id[:12]}.json"
        assert export_path.exists()
    ...
    async def test_cmd_stash_pop(self) -> None:
        from obscura.cli.commands import cmd_stash, cmd_pop, _stash_stack
        _stash_stack.clear()
        ctx = _make_mock_ctx()
        ctx.message_history = [("user", "hello"), ("assistant", "hi")]
    ...
    async def test_cmd_pop_empty(self) -> None:
        from obscura.cli.commands import cmd_pop, _stash_stack
        _stash_stack.clear()
        ctx = _make_mock_ctx()
    ...
    async def test_cmd_rewind_with_changes(self, tmp_path: Path) -> None:
        ...
        ctx._file_changes = [{"path": str(f), "original": "original", "modified": "changed"}]
...
class TestToolExecution:
    ...
    async def test_edit_with_diff_output(self, tmp_path: Path) -> None:
        ...
        result = await specs["edit_text_file"].handler(
            path=str(f), old_text="world", new_text="planet"
        )
        data = json.loads(result)
        assert data["ok"] is True
        assert "diff" in data
    ...
    async def test_grep_output_modes(self, tmp_path: Path) -> None:
        ...
        for mode in ["content", "files_with_matches", "count"]:
            result = await specs["grep_files"].handler(
                pattern="line1", path=str(tmp_path), output_mode=mode
            )
            data = json.loads(result)
    ...
    async def test_task_create_and_list(self) -> None:
        ...
        # Create.
        result = await specs["task_create"].handler(subject="Test task", description="Testing")
        data = json.loads(result)
        assert data["ok"] is True
    ...
    async def test_notebook_edit_nonexistent(self) -> None:
        ...
        result = await specs["notebook_edit"].handler(
            notebook_path="/nonexistent.ipynb", cell_index=0
        )
        data = json.loads(result)
...
class TestPermissionIntegration:
    ...
    def test_plan_mode_blocks_in_evaluate(self) -> None:
        ...
        for tool in ["write_text_file", "edit_text_file", "run_shell", "run_command",
                      "remove_path", "move_path", "git_commit"]:
            d = engine.evaluate(tool)
            assert not d.allowed, f"{tool} should be blocked in PLAN mode"

        # These should be allowed.
        for tool in ["read_text_file", "grep_files", "find_files", "git_status", "web_search"]:
            d = engine.evaluate(tool)
    ...
    def test_dangerous_overrides_bypass(self) -> None:
        ...
        for tool, args in dangerous_commands:
            ...
            assert not d.allowed, f"Dangerous command should be blocked even in BYPASS: {args}"
...
class TestSessionUtilsIntegration:
    def test_register_and_list(self) -> None:
        from obscura.core.session_utils import register_session, unregister_session, list_active_sessions
    ...
    def test_concurrent_detection(self) -> None:
        from obscura.core.session_utils import register_session, unregister_session, check_concurrent_sessions
...
class TestDiffIntegration:
    ...
    def test_diff_side_by_side(self) -> None:
        from obscura.cli.app.diff_engine import DiffEngine, FileChange
        engine = DiffEngine()
