...
class TestRecursiveDiscovery:
    ...
    def test_discover_targets_finds_root(
        ...
        assert mock_repo in repo_paths, \
            f"Root should be a target, got {repo_paths}"
    ...
    def test_root_target_has_files(
        ...
        assert "agent.md" in dest_names, \
            f"Root should have agent.md, got {dest_names}"
    ...
    def test_partview_target_has_own_skills(
        ...
        pv = [t for t in targets
              if t.repo_path == mock_repo / "platform" / "partview_core"][0]
