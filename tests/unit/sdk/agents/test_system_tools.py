...
class TestRunNpx:
    ...
    async def test_timeout_returns_error_payload(self) -> None:
        ...
        proc.wait = AsyncMock()
        with patch(
            "sdk.agent.system_tools.asyncio.create_subprocess_exec", return_value=proc
        ):
...
class TestRunCommand:
    ...
    async def test_empty_denied_env_disables_default_denylist(self) -> None:
        ...
        with patch.dict(
            ...
            "sdk.agent.system_tools.asyncio.create_subprocess_exec",
            return_value=proc,
        ):
...
class TestOpsAndSecurityTools:
    ...
    async def test_manage_crontab_list_without_binary(self) -> None:
        with patch("sdk.agent.system_tools.shutil.which", return_value=None):
    ...
    async def test_manage_crontab_add_requires_fields(self) -> None:
        with patch("sdk.agent.system_tools.shutil.which", return_value="/usr/bin/crontab"):
