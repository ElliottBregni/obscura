...
class TestMCPToolProvider:
    ...
    async def test_starts_backend_and_registers_tools(self) -> None:
        ...
        with patch("sdk.backends.mcp_backend.MCPBackend") as MockBackend:
            backend = AsyncMock()
...
class TestA2ARemoteToolProvider:
    ...
    async def test_registers_remote_agents_as_tools(self) -> None:
        ...
        with patch("sdk.agent.tool_providers.A2AClient") as MockA2AClient, patch(
            "sdk.agent.tool_providers.register_remote_agent_as_tool",
            return_value=spec,
        ):
