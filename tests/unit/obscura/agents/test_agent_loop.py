...
class TestAgentLoopToolExecution:
    ...
    @pytest.mark.asyncio
    async def test_parallel_wrapper_expands_nested_tool_uses(self) -> None:
        calls: list[str] = []
        ...
        turn1 = _make_tool_call_chunks(
            "parallel",
            {
                "tool_uses": [
                    {
                        "recipient_name": "functions.report_intent",
                        "parameters": {"intent": "Searching web"},
                    },
                    {
        ...
        backend = MockBackend([turn1, turn2])
        loop = AgentLoop(backend, _make_registry(web_search_spec))

        events: list[AgentEvent] = [e async for e in loop.run("search")]
        tool_results = [e for e in events if e.kind == AgentEventKind.TOOL_RESULT]

        assert calls == ["obscura"]
        assert any("search:obscura" in e.tool_result for e in tool_results)
...
class TestToolCallParsing:
    ...
    def test_parse_functions_prefixed_tool_name(self) -> None:
