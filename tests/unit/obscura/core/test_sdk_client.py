...
class TestReasoningDefaults:
    def test_defaults_reasoning_effort_for_gpt_models(self) -> None:
        client = ObscuraClient.__new__(ObscuraClient)
        client._model = "gpt-5"
        out = client._apply_default_reasoning_kwargs({})  # pyright: ignore[reportPrivateUsage]
        assert out["reasoning_effort"] == "medium"

    def test_does_not_default_for_non_gpt_models(self) -> None:
        client = ObscuraClient.__new__(ObscuraClient)
    ...
    def test_explicit_responses_reasoning_wins(self) -> None:
        client = ObscuraClient.__new__(ObscuraClient)
        client._model = "gpt-5"
        out = client._apply_default_reasoning_kwargs(  # pyright: ignore[reportPrivateUsage]
