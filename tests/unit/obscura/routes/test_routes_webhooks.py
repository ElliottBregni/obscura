...
class TestSSRFGuard:
    """SOC2 finding E2 — outbound webhook URLs must pass the SSRF guard."""
    ...
    def test_create_rejects_blocked_url(self, client: TestClient, url: str) -> None:
        resp = client.post(
            "/api/v1/webhooks",
            json={"url": url, "events": ["agent.spawn"]},
        )
        assert resp.status_code == 422
        assert "webhook url" in resp.json()["detail"].lower()

    @pytest.mark.asyncio
    @patch("httpx.AsyncClient")
    async def test_trigger_skips_url_that_now_resolves_private(
        self,
        mock_async_cls: Any,
    ) -> None:
        """Stored URL whose DNS rotated to a private IP is dropped silently."""
        from obscura.routes.webhooks import (
            WebhookConfig,
            get_webhooks_store,
            trigger_webhooks,
        ...
        store = get_webhooks_store()
        store.clear()
        # Bypass create-time validation by writing the WebhookConfig directly
        # — simulates a URL that was once public but now resolves to RFC1918.
        store["rebound"] = WebhookConfig(
            webhook_id="rebound",
