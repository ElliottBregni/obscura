...
from obscura.cli import main, send_message, _cli_confirm, _discover_mcp, _track_file_event
...
class TestDiscoverMCP:
    ...
    def test_returns_empty_when_no_servers(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(
            "obscura.cli.__init__.discover_mcp_servers",
            lambda: [],
            raising=False,
        )
...
class TestSendMessage:
    ...
    async def test_basic_text_response(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("obscura.cli.__init__.console", MagicMock())
        monkeypatch.setattr("obscura.cli.__init__.trace_mod", MagicMock())
        monkeypatch.setattr(
            "obscura.cli.__init__._estimate_tokens", lambda x: 100
        )
        monkeypatch.setattr(
            "obscura.cli.__init__.update_token_usage", lambda **kw: None
        )
    ...
    async def test_auto_compact_on_threshold(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("obscura.cli.__init__.console", mock_console)
        monkeypatch.setattr("obscura.cli.__init__.trace_mod", MagicMock())
        monkeypatch.setattr(
            "obscura.cli.__init__._estimate_tokens", lambda x: token_count
        )
        monkeypatch.setattr(
            "obscura.cli.__init__.update_token_usage", lambda **kw: None
        )
        ...
        monkeypatch.setattr("obscura.cli.__init__.cmd_compact", _fake_compact)
    ...
    async def test_vector_memory_augmentation(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "obscura.cli.__init__.search_relevant_context",
            lambda vs, q, top_k: "[memory] relevant info",
        )
        monkeypatch.setattr(
            "obscura.cli.__init__.auto_save_turn",
            lambda *a, **kw: None,
        )
