...
from sdk.vector_memory import VectorMemoryStore, cosine_similarity, simple_embedding
...
class TestVectorMemoryStore:
    def test_set_and_get(self, test_user: AuthenticatedUser, temp_db: Path) -> None:
        store = VectorMemoryStore(test_user, db_path=temp_db)
    ...
    def test_get_missing(self, test_user: AuthenticatedUser, temp_db: Path) -> None:
        store = VectorMemoryStore(test_user, db_path=temp_db)
    ...
    def test_delete(self, test_user: AuthenticatedUser, temp_db: Path) -> None:
        store = VectorMemoryStore(test_user, db_path=temp_db)
    ...
    def test_search_similar_basic(self, test_user: AuthenticatedUser, temp_db: Path) -> None:
        store = VectorMemoryStore(test_user, db_path=temp_db)
    ...
    def test_search_similar_with_namespace(self, test_user: AuthenticatedUser, temp_db: Path) -> None:
        store = VectorMemoryStore(test_user, db_path=temp_db)
    ...
    def test_search_similar_threshold(self, test_user: AuthenticatedUser, temp_db: Path) -> None:
        store = VectorMemoryStore(test_user, db_path=temp_db)
...
class TestSemanticMemoryMixin:
    ...
    def test_remember_creates_entry(self, test_user: AuthenticatedUser, temp_db: Path) -> None:
    ...
    def test_recall_finds_similar(self, test_user: AuthenticatedUser, temp_db: Path) -> None:
        store = VectorMemoryStore(test_user, db_path=temp_db)
        
        # Simulate memories
        store.set("mem1", "Python async/await is for concurrency", namespace="default:semantic")
        store.set("mem2", "JavaScript promises are asynchronous", namespace="default:semantic")
        store.set("mem3", "Cooking pasta requires boiling water", namespace="default:semantic")
